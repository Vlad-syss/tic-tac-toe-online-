...
export const ProfileBoard: FC<ProfileBoardProps> = ({
	...
	return (
		<div className='pb-3 mb-3 border-b border-slate-200 dark:border-slate-800'>
			<div className='flex relative flex-col sm:flex-row justify-between gap-3 sm:gap-0 bg-slate-100 dark:bg-slate-800/70 p-3 sm:p-4 px-5 rounded-lg shadow-md border border-slate-200 dark:border-slate-700'>
				{userIds.map((userId, i) => {
					...
					return (
						...
						>
							{elimination && (
								<div className='absolute -top-2 left-1/2 -translate-x-1/2 z-10 text-xs font-bold px-2 py-0.5 rounded-full bg-slate-800 dark:bg-slate-200 text-white dark:text-slate-900'>
									#{elimination.position}
								</div>
							)}
							<div className='flex flex-col items-center justify-center w-24 h-12 pt-2 pb-1 border-2 border-slate-300 dark:border-slate-600 rounded-lg shadow-slate-200/40 dark:shadow-slate-900/40 text-lg font-semibold text-slate-600 dark:text-slate-300 select-none'>
								<h3 className='text-xs text-slate-500 mr-2'>⏳ Timer:</h3>
								<span className='text-slate-700 dark:text-slate-300 font-bold text'>
